...
case "$TYPE" in
    hourly)
        FILENAME="obsidian_hourly_$(date '+%Y%m%d_%H%M%S').tar.gz"
        ;;
    daily)
        FILENAME="obsidian_daily_$(date '+%Y%m%d').tar.gz"
        ;;
    permanent)
        FILENAME="obsidian_permanent_$(date '+%Y%m').tar.gz"
        ;;
esac
...
# 백업 생성
tar -czf "$BACKUP_DIR/$FILENAME" -C "$VAULT_DIR" .
...
if [ -n "$RETENTION_HOURS" ] && [ "$RETENTION_HOURS" -gt 0 ]; then
    ...
    DELETED=$(find "$BACKUP_DIR" -name "obsidian_${TYPE}_*.tar.gz" -mmin +${RETENTION_MINUTES} -print)

    if [ -n "$DELETED" ]; then
        echo "$DELETED" | while read -r f; do
            echo "[$(date '+%Y-%m-%d %H:%M:%S')] [$TYPE] 삭제: $(basename "$f")"
        done
        find "$BACKUP_DIR" -name "obsidian_${TYPE}_*.tar.gz" -mmin +${RETENTION_MINUTES} -delete
    fi

    COUNT=$(find "$BACKUP_DIR" -name "obsidian_${TYPE}_*.tar.gz" | wc -l)
